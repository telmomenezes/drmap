...
public class JFastEMD {
    ...
    static private long emdHatImplLongLongInt(Vector<Long> Pc, Vector<Long> Qc,
        ...
        Vector<List<EdgeEMD>> c = new Vector<List<EdgeEMD>>();
        for (int i = 0; i < b.size(); i++) {
            c.add(new LinkedList<EdgeEMD>());
        }
        for (int i = 0; i < N; i++) {
            if (b.get(i) == 0)
                continue;
            for (int j = 0; j < N; j++) {
                ...
                if (C.get(i).get(j) == maxC)
                    continue;
                c.get(i).add(new EdgeEMD(j + N, C.get(i).get(j)));
            }
        }
        ...
        for (int i = 0; i < N; ++i) {
            c.get(i).add(new EdgeEMD(THRESHOLD_NODE, 0));
        }
        for (int j = 0; j < N; ++j) {
            c.get(THRESHOLD_NODE).add(new EdgeEMD(j + N, maxC));
        }

        // artificial arcs - Note the restriction that only one edge i,j is
        // artificial so I ignore it...
        for (int i = 0; i < ARTIFICIAL_NODE; i++) {
            c.get(i).add(new EdgeEMD(ARTIFICIAL_NODE, maxC + 1));
            c.get(ARTIFICIAL_NODE).add(new EdgeEMD(i, maxC + 1));
        }
        ...
        Vector<List<EdgeEMD>> cc = new Vector<List<EdgeEMD>>();
        for (int i = 0; i < bb.size(); i++) {
            cc.add(new LinkedList<EdgeEMD>());
        }
        for (int i = 0; i < c.size(); i++) {
            if (nodesNewNames.get(i) == REMOVE_NODE_FLAG)
                continue;
            for (EdgeEMD it : c.get(i)) {
                if (nodesNewNames.get(it._to) != REMOVE_NODE_FLAG) {
                    cc.get(nodesNewNames.get(i)).add(
                            new EdgeEMD(nodesNewNames.get(it._to), it._cost));
                }
            }
        }
